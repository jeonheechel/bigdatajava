...
implements Reducer<IntWritable, IntWritable, Text, IntWritable>{
	...
	private Text getWeek(int idx) {
		Text week = new Text();
		/*
		 * if(idx==1) week.set("월요일"); else if(idx==2) week.set("화요일"); else if(idx==3)
		 * week.set("수요일"); else if(idx==4) week.set("목요일"); else if(idx==5)
		 * week.set("금요일"); else if(idx==6) week.set("토요일"); else if(idx==7)
		 * week.set("일요일");
		 */
		
		final String []dayArr = {"월요일", "화요일","수요일",
				"목요일", "금요일", "토요일", "일요일"};
