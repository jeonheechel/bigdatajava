	...
	public WritePage() {
		...
		completeButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				daoB2.insert(numCount[0] + 1, m.getId(), dto.getNicname(), inputTitleField.getText(), inputContentFiled.getText(), today);
				
				int result = JOptionPane.showConfirmDialog(null, "게시글 작성을 완료 하시겠습니까?", "완료",
						JOptionPane.YES_NO_OPTION);
				if (result == JOptionPane.CLOSED_OPTION) {
					// If you have closed the dialog without selecting.
				} else if (result == JOptionPane.YES_OPTION) {
					// If Yes is selected
					JOptionPane.showMessageDialog(null, "게시글 작성이 완료 되었습니다!");
		...
		cancelButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				int result = JOptionPane.showConfirmDialog(null, "글 작성을 취소하시겠습니까?", "취소",
						JOptionPane.YES_NO_OPTION);
				if (result == JOptionPane.CLOSED_OPTION) {
					// If you have closed the dialog without selecting.
				} else if (result == JOptionPane.YES_OPTION) {
		...
		backwardBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				int result = JOptionPane.showConfirmDialog(null, "작성 중이던 글은 저장 되지 않습니다. 이전 페이지로 돌아가시겠습니까?" + 
